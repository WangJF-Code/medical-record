...
public class DetectInflammatoryMediaController {
    ...
    @ApiOperation("列表信息")
    @PostMapping("/list")
    public ApiResult<List<DetectInflammatoryMedia>> list(@ApiParam(required = true, value = "type") Integer type,
                                                         @ApiParam(required = true, value = "dataId") String dataId) {
        List<DetectInflammatoryMedia> list = service.getList(type, dataId);
        return ApiResultUtil.okay(list);
    }

    @ApiOperation("详细信息")
    @GetMapping("/{id}")
    public ApiResult<DetectInflammatoryMedia> info(@ApiParam(required = true, value = "id") @PathVariable Integer id) {
        DetectInflammatoryMedia info = service.get(id);
        return ApiResultUtil.okay(info);
    }

    @ApiOperation("修改信息")
    @PutMapping
    public ApiResult update(@RequestBody DetectInflammatoryMedia updateDTO) {
    ...
    @ApiOperation("创建信息")
    @PostMapping
    public ApiResult create(@RequestBody DetectInflammatoryMedia createDTO) {
